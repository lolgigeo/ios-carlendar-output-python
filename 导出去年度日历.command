...
# 显示导出信息
echo "本工具将导出${last_year}年度（${start_date} 至 ${end_date}）的所有日历日程。"
echo ""
...
# 获取输出文件名
echo ""
read -p "请输入输出文件名 (默认为: ${last_year}年日历.csv): " output_file

# 如果没有指定输出文件名，则使用默认名称
if [ -z "$output_file" ]; then
    output_file="${last_year}年日历.csv"
fi
...
# 执行导出命令
echo ""
echo "开始导出${last_year}年度日历日程，请稍候..."
